...
//this will get the entire message from the client.
{
	...
	while (strstr(message, "@@") == NULL) // keep executing until we find null terminator for transmission
	{
		...
		if(r == -1) {printf("OTP_ENC: r == -1\n"); break;}
		if(r == 0) {printf("OTP_ENC: r == 0\n"); break;}
	}
...
int main(int argc, char *argv[])
{
	...
	serverHostInfo = gethostbyname("10.0.0.172"); // get host info
...
//	program = NULL;
	...
	{
	//	printf("OTP_ENC: Connected to correct server\n");
	}

	else
	//if it is not the right server, let the server know then exit
	{
		fprintf(stderr, "OTP_ENC: ERROR: Connected to incorrect server!\nExiting...\n");
		exit(1);
	}
	...
	sendall(socketFD, "bs@@\0", 5); //this is just so the client or server doesnt jump ahead with out permission
